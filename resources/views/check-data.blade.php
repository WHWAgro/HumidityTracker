				...
				<div class="page-wrap">
					<div class="influ-strip">
						<div class="influ-search">
							<form action="{{ url('forecast_data') }}"method="POST">
								@csrf
								<div class="field_section">
									<label>
										<input type="text" class="field_name" name="field_name" placeholder="Buscar Id de la Parcela" value="{{ $requested_field_name ?? old('field_name') }}" required="">
										<img src="{{ asset('images/search.png') }}"alt="">
									</label>
									<div class="field_name_section"></div>
								</div>
								<label class="pr-0" for="datepicker">
									<input type="date" name="date" placeholder="{{ $requested_date == '' ? 'dd-mm-yyyy' : date('d-m-Y',strtotime($requested_date)) }}" id="datepicker" required="" onfocus="this.showPicker()">
								</label>
								<input type="submit"value="Enviar"class="influ-search-btn">
								<!-- <a href=""class="influ-search-btn"><img src="{{ asset('images/export.png') }}"alt="">Export CSV</a> -->
								<span style="cursor:pointer" data-href="{{ url('export-csv') }}" id="export" class="influ-search-btn" onclick ="exportTasks (event.target);"><img src="{{ asset('images/export.png') }}"alt="">Exportar CSV</span>
							</form>
						</div>
					</div>
						...
						<div class="page-map">
							...
							<div class="icon">
								<div class="tooltip-box">
									<a class="show-modal" data-toggle="modal" data-target="#myModal"><img src="{{ asset('images/tooltip-edit.png') }}" alt=""></a>
							    ...
							    <div id="map"></div>
							<!-- <img src="{{ asset('images/data-map.jpg') }}" alt=""> -->
						</div>
							...
							<div class="table-responsive">
								<table>
									...
									@foreach($field_data as $field)
										...
										?>
									<tr id="humidity_{{ $field->id }}">
										<td>{{ date("M d,Y",strtotime($field->humidity_date)) }}</td>
										<td>{{ $field->field_name }}</td>
										<td>{{ $field->field_code }}</td>
										<td>{{ $field->field_alias }}</td>
										<td>{{ $field->razon_social }} </td>
										<td>{{ $field->latitude }} °N</td>
										<td>{{ $field->longitude }}°W</td>
										<td class="humidity_update">{{ $field->humidity }}%</td>
										<td>
											<a href="javascript:void(0)" class="show-modal" data-id="{{ $field->id }}"><img src="{{ asset('images/edit.png') }}" alt=""></a>
											@if(auth()->user()->id == $field['user_id'])
												<a href="javascript:void(0)" class="delete_humidity" data-id="{{ $field->id }}"><img src="{{ asset('images/delete.png') }}" alt=""></a>
											@endif
										</td>
									</tr>
									@endforeach
